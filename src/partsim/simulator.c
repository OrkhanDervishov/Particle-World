...
Rect SimulateChunkRect(Chunk* chunk, Rect rect){
    ...
    for(int i = rect.y; i < rect.y + rect.h; i++){
        ...
        for(int j = nj; mov ? j >= rect.x : j < rect.x + rect.w; j += add){
            ...
            if(CHUNK_GET(chunk->state, j, i) <= 0) continue;
            
            // map[y][x].p.y >= rows-1 || 
            if(typeFuncList[CHUNK_GET(chunk->type, j, i)] == NULL) {continue;}
            
            // Particle logic
            CHUNK_GET(chunk->state, j, i) = 0;
            bool changed = typeFuncList[CHUNK_GET(chunk->type, j, i)](x, y);
