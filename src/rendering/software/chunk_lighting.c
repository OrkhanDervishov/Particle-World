...
void init_lightmap_image(int chunk_w, int chunk_h, int partSize){
    ...
    pnt_create_image(&chunk_lightmap, chunk_w, chunk_h);
    pnt_create_fimage(&chunk_lightmap_f, chunk_w, chunk_h, 0x000000FF, 0x0000FF00, 0x00FF0000, 0xFF000000);
}

void free_lightmap_image(){
    pnt_delete_fimage(&chunk_lightmap_f);
    pnt_delete_image(&chunk_lightmap);
}
...
void draw_chunk_lightmap(Image lightmap, Chunk* chunk, int x, int y){
    ...
    pnt_blit(lightmap, chunk_lightmap, x, y);
} 
...
void blur_lightmap(Image dest, int times){
    if(times == 0) return;
    pnt_gaussian_blur_3x3(dest, chunk_lightmap);
    for(int i = 1; i < times; i++){
        pnt_gaussian_blur_3x3(dest, dest);
    }
}
...
void blur_lightmap2(Image* dest, Image light_map, int times){
    ...
    Image dest2;
    dest2.buffer = NULL;
    pnt_create_similar(&dest2, *dest);
    bool must_copy = FALSE;

    pnt_gaussian_blur_3x3(*dest, light_map);
    for(int i = 1; i < times; i++){
        // gaussian_blur_3x3(dest, dest);
        if(!must_copy){
            pnt_gaussian_blur_3x3(dest2, *dest);
            must_copy = TRUE;
        }
        else{
            pnt_gaussian_blur_3x3(*dest, dest2);
            must_copy = FALSE;
        }
    }
...
void blur_lightmap_strong(Image* dest, Image light_map, int kernel_size, int times){
    ...
    pnt_gaussian_blur_horizontal(*dest, light_map, kernel_size);
    pnt_gaussian_blur_vertical(dest2, *dest, kernel_size);
    for(int i = 1; i < times; i++){
        pnt_gaussian_blur_horizontal(*dest, dest2, kernel_size);
        pnt_gaussian_blur_vertical(dest2, *dest, kernel_size);
    }

    pnt_copy_image_content(*dest, dest2);
    pnt_delete_image(&dest2);
}